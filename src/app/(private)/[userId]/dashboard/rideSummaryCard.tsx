...
import {ChevronRight, Navigation, PlayCircleIcon, Route, Ruler} from "lucide-react";
import {Button} from "@/components/ui/button";

export const RideDetailsSummaryCard = ({ride}: { ride: RideFormSchema }) => {
    return (
        <Card className="rounded-none shadow-none">
            ...
            <CardContent>
                ...
                <div className="flex justify-between items-center">
                    <div>
                        {ride.attendanceList && ride.attendanceList.length > 0 ? (<p>{ride.attendanceList.length}</p>) : <p className="text-gray-400">Be the first to join!</p>}
                    </div>
                    <Button>Join Ride <ChevronRight /></Button>
                </div>
            </CardContent>
